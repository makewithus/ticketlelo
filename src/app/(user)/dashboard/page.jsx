...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && !user) {
      console.log("⚠️ No user found, redirecting to homepage");
      router.push("/");
    }
      ...
      <header className="fixed top-0 inset-x-0 z-50 glass-dark">
        <nav className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-3 items-center">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/50 border border-emerald-500/30">
            ...
            <Button
              ...
              variant="outline"
              className="h-10 gap-2 border-emerald-500/30 text-emerald-400 hover:text-white hover:border-red-500/50 hover:bg-red-500/10 transition-all"
            >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-10 opacity-0 animate-fade-in-up">
            <h1 className="text-4xl font-extrabold mb-2 text-white">
              Welcome, <span className="text-emerald-400">{user.fullName}</span>
              !
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
            {[
              ...
              <div
                key={stat.label}
                className={`p-5 rounded-2xl border bg-black/50 border-emerald-500/30 opacity-0 animate-fade-in-up stagger-${i + 1}`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                  filter === f.key
                    ...
                    : "text-slate-400 hover:text-slate-300 border border-transparent hover:border-emerald-500/20 hover:bg-black/50"
                }`}
              >
                {f.label}
              </button>
          ...
          {filteredRegistrations.length === 0 ? (
            <div className="text-center py-20 opacity-0 animate-fade-in">
              ...
              {filter === "All" && (
                <Link href="/">
                  <Button className="h-11 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white shadow-lg shadow-emerald-500/25 gap-2">
                    Browse Events
                  </Button>
                </Link>
              )}
