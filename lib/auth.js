...
export const loginWithEmailAndPhone = async (email, phone) => {
  try {
    ...
    try {
      ...
      if (loginError.message?.includes("invalid-credential") || loginError.message?.includes("wrong-password")) {
        console.log("📧 Stored password doesn't match Firebase Auth - sending password reset email");
        
        try {
          // Send password reset email
          await sendPasswordResetEmail(auth, email);
          throw new Error(
            "Password mismatch detected. We've sent a password reset email to " + email + ". Please reset your password and try again, or register for a new event to update your account."
          );
        } catch (resetError) {
