  ...
  try {
    ...
    if (userId) {
      ...
      if (userDoc.exists()) {
        ...
        console.log(
          `✅ Incremented eventsCreated for ${userId}: ${currentCount} → ${currentCount + 1}`,
        );
      }
    }
    ...
    if (eventDoc.exists()) {
      ...
      if (organizerId) {
        ...
        if (userDoc.exists()) {
          ...
          console.log(
            `✅ Decreased eventsCreated for organizer ${organizerId}: ${currentCount} → ${newCount}`,
          );
        }
      }
      ...
      if (!formsSnapshot.empty) {
        ...
        await batch.commit();
        console.log(
          `✅ Deleted ${deletedFormsCount} custom form(s) associated with event ${eventId}`,
        );
      }

      // Note: We keep registrations for history/audit purposes
      console.log(
        `ℹ️ Registrations for event ${eventId} are kept for audit purposes`,
      );
    } else {
